...
GET_NEARBY_SCHOOL_RATING = '''//div[@class="nearby-schools-rating"]/span/text()'''

GET_SOLD_PRICE = '''//div[@class="main-row status-icon-row recently-sold-row home-summary-row"]/span[@class=""]/text()'''
...
def get_zpid_by_zipcode(zipcode):
    ...
    try:
        nextpage = tree.xpath(GET_NEXT_PAGE)[0]
    except Exception:
        pass
    ...
    while nextpage != None:
    	try:
            session_requests = requests.session();
            response = session_requests.get(nextpage, headers=getHeaders())
            tree = html.fromstring(response.content)
            zpids = zpids+tree.xpath(SEARCH_XPATH_FOR_ZPID)
        except Exception:
            pass
...
def get_property_by_zpid(zpid):
    ...
    list_price = None
    try:
        list_price_raw = tree.xpath(GET_SOLD_PRICE)
    ...
    rating = None
    try:
        rating = tree.xpath(GET_NEARBY_SCHOOL_RATING)
        school_rating = (float(rating[0])+float(rating[1])+float(rating[2]))/3
    except Exception:
        pass
